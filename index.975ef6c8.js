...
let form, output, outputSeq, lastSeq = [], brakStron, stronData;
...
function visualize() {
    ...
    const seq = form.input.value.trim().split(" ").map((e)=>+e);
    ...
    for(let i = 0; i < seq.length; i++){
        const element = +seq[i];
        if (!funcAlg) opt(element, mem, queue, seq, i);
...
},{}]},["farZc","8lqZg"], "8lqZg", "parcelRequire94c2")
